#!/usr/bin/env node
/* server/migrations/run-migration-update-flags.js */
'use strict';
...
async function runMigration() {
  ...
  try {
    ...
    // Ler arquivo SQL
    const sqlPath = path.join(__dirname, '004_update_empresa_flags.sql');
    if (!fs.existsSync(sqlPath)) {
      throw new Error(`Arquivo de migração não encontrado: ${sqlPath}`);
    }

    console.log(`📄 Executando migração: 004_update_empresa_flags.sql`);
