#!/usr/bin/env node
/* server/migrations/run-migration-cpf-empresa.js */
'use strict';
...
async function runMigration() {
  ...
  try {
    ...
    // Ler arquivo SQL
    const sqlPath = path.join(__dirname, '007_add_cpf_empresa_usuarios.sql');
    if (!fs.existsSync(sqlPath)) {
      throw new Error(`Arquivo de migração não encontrado: ${sqlPath}`);
    }

    console.log(`📄 Executando migração: 007_add_cpf_empresa_usuarios.sql`);
    ...
    constraints.rows.forEach(constraint => {
      const type = constraint.contype === 'f' ? 'Foreign Key' : 'Unique';
