#!/usr/bin/env node
/* server/migrations/run-migration-usuarios.js */
'use strict';
...
async function runMigration() {
  ...
  try {
    ...
    // Ler arquivo SQL
    const sqlPath = path.join(__dirname, '005_recreate_usuarios_table.sql');
    if (!fs.existsSync(sqlPath)) {
      throw new Error(`Arquivo de migração não encontrado: ${sqlPath}`);
    }

    console.log(`📄 Executando migração: 005_recreate_usuarios_table.sql`);
    ...
    const checkTable = await client.query(`
      SELECT
        table_name,
        (SELECT COUNT(*) FROM usuarios) as record_count
      FROM information_schema.tables
      WHERE table_name = 'usuarios'
    `);

    if (checkTable.rows.length > 0) {
      console.log(`\n📊 Status da tabela 'usuarios':`);
