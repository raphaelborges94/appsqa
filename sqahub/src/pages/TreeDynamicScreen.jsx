...
export default function TreeDynamicScreen() {
  ...
  // Buscar screen - ajustado para usar TreeScreenDefinition.get
  ...
  // Buscar records - ajustado para usar base44.entities.list (padrão CRUD)
  const { data: records = [], isLoading: recordsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Transformar para formato esperado: { id, data: {...campos} }
  ...
  // Mutations - ajustadas para usar base44.entities diretamente
  ...
  const buildTree = (records, parentId = null) => {
    return records
      .filter(r => {
        const paiValue = r.data[screen.campo_pai];
        if (parentId === null) {
          return !paiValue || paiValue === null || paiValue === 0;
        }
      ...
      .sort((a, b) => {
        if (screen.campo_codigo) {
          const codeA = a.data[screen.campo_codigo] || '';
          const codeB = b.data[screen.campo_codigo] || '';
  ...
  const handleDelete = (record) => {
    ...
    const hasChildren = records.some(r => r.data[screen.campo_pai] === record.id);

    if (hasChildren) {
      toast.error('Não é possível excluir um item que possui filhos');
      return;
    }

    const nome = record.data[screen.campo_nome] || 'este registro';
      ...
      <div className="flex-1 overflow-hidden bg-gradient-to-br from-slate-50 to-slate-100 p-6">
        <div className="h-full max-w-6xl mx-auto">
          <div className="grid grid-cols-12 gap-6 h-full">
            ...
            {/* Coluna Formulário/Detalhes - 7 colunas */}
